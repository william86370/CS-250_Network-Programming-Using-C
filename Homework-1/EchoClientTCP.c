...
//
//
//  Created by william wright on 10/9/18.
//
...
int main(int argc, char *argv[])
{
    ...
    if ((sock_Descr =  socket(PF_INET , SOCK_STREAM , IPPROTO_TCP)) < 0) {
        DieWithError("Error with Creating Socket");
    }
    ...
    if (connect(sock_Descr, (struct sockaddr *) &echoServAddr, sizeof(echoServAddr)) < 0){
        DieWithError("error creating connection");
    }
    ...
    {
         bytes = 0;
        if ((bytes = recv(sock_Descr, echoBuffer , RCVBUFSIZE-1 , 0)) <1) {
            DieWithError("Error Receiving Bytes");
        }
